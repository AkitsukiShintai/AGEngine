...
void Graphics::CreateInstance() {
	...
	appInfo.pApplicationName = window->applicationName.c_str();
...
#else
	if (window->winWindowHandle != nullptr)
	{
		VkWin32SurfaceCreateInfoKHR createInfo = {};
		createInfo.sType = VK_STRUCTURE_TYPE_WIN32_SURFACE_CREATE_INFO_KHR;
		createInfo.hwnd = window->winWindowHandle;
		createInfo.hinstance = window->winAppliocationInstance;
...
void Graphics::CreateSwapChain() {
	...
	for (size_t i = 0; i < images.size(); ++i)
	{
		...
		swapChainImages[i].width = window->width;
		swapChainImages[i].height = window->height;
