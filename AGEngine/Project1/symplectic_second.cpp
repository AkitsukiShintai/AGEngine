//#define SIMPLECTIC_SECOND
#ifdef SIMPLECTIC_SECOND
#include <iostream>
#include <iomanip>
#include <vector>
#include <string>
...
Pair SymiImpIntegration(const Pair& inValue, float h, DifferentialFunc diffFuncX, DifferentialFunc diffFuncY) {
	Pair result;
	result.y = inValue.y + h * diffFuncY(inValue);
	result.x = inValue.x + h * diffFuncX(result);
...
int main() {
	...
	{
		inValue = SymiImpIntegration(inValue, 0.1f, DiffeOfTheta, DiffeOfW);
