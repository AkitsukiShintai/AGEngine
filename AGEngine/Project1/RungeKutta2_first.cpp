...
Pair RK2Integration(const Pair& inValue, float h, DifferentialFunc diffFunc) {
	Pair y12 = EulerIntegration(inValue, 0.5f * h, diffFunc);
	float k1 = h * diffFunc(inValue);
	Pair newPair = { inValue.x + 0.5f * h, inValue.y + 0.5f * k1 };
	float k2 = h * diffFunc(newPair);
	return Pair{ inValue.x + h, inValue.y + k2 };
}
...
int main() {
	...
	{
		inValue = RK2Integration(inValue, 0.5f, FirstOrderDifferential);
